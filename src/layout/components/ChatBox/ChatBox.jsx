...
import { Phone, Video, Info, Mic, Image as Img, Sticker, Smile, ThumbsUp, FolderGit, X } from "lucide-react";
...
function ChatBox({ className, onClick }) {
    ...
    const currentChat = useMemo(() => {
        return rep && currentChatId ? currentChatId.data_Message.find(msg => msg.id === rep) : null;
    }, [rep]);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            const foundChat = arrContent.find((item) => item.id === Number(chatId));
            setCurrentChatId(foundChat || null);
            setLoading(false);
        }, 900);
                        ...
                        rep && (
                            <div className={cx("footer-rep")}>
                                <div className={cx("footer-rep-content")}>
                                    <div className={cx("footer-rep-title")}>Đang trả lời {currentChat.id_user == UserRoot.id ? "chính mình" : currentChat.user}</div>
                                    <div className={cx("footer-rep-text")}>{currentChat.content_image ? "Hình ảnh": currentChat.titlefile ? "File đính kèm" : currentChat.content}</div>
                                </div>
